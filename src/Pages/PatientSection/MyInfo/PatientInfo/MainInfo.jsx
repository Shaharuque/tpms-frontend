...
import { getMyInfoDetails } from "../../../../features/PatientPortal/MyInfo_redux/myInfoSlice";
...
const MainInfo = () => {
  ...
  const data = useSelector((state) => state?.myInfo);
  console.log("Initial Data Coming from database", data);
  const patient_details = data?.infoDetails?.patientInitialInfo;
  ...
  //MyInfo Details thunk action dispatched here
  useEffect(() => {
    // action dispatched
    dispatch(getMyInfoDetails(token));
  }, [id, dispatch, token]);
