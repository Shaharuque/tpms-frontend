...
const ProviderEditClearence = ({ handleClose, open, clearenceInfo }) => {
  ...
  const { data: clearenceData, isLoading, isSuccess } = useProviderclearenceInfoQuery({ token, id: clearenceInfo?.id });

  //Update clearence info data api
  const [updateClearence, { isSuccess: updateSuccess, isError: updateError }] = useProviderupdateClearenceMutation();

  const { clearance_name, clearance_date_issue, clearance_date_exp, clearance_applicable } = clearenceData?.employeeClearance || {}; //api tey bhul
  ...
  }, [reset, clearance_name, clearance_applicable, clearance_date_exp, clearance_date_issue]);
...
export default ProviderEditClearence;
