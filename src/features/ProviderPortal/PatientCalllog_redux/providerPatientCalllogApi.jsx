...
export const patientDocumentApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    //get calllogs
    getProviderPatientLogs: builder.query({
      query: ({ token, id }) => ({
        url: `/provider/patients/get/patient/calllog/list/${id}`,
        method: "GET",
        headers: {
      ...
      providesTags: ["ProviderPortalLogs"],
    }),
    // // Add documents
...
export const { useGetProviderPatientLogsQuery } = patientDocumentApi;
